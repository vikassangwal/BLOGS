...
export async function GET(request: NextRequest) {
  try {
    ...
    const [totalKeywords, pendingKeywords, usedKeywords, totalAutoPosts, logs] = await Promise.all([
      ...
      prisma.autoBlogLog.findMany({ orderBy: { createdAt: 'desc' }, take: 20 })
    ]);
    ...
    return NextResponse.json({ error: 'Failed to fetch auto-blog data' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (pendingKeyword) {
      ...
      // 2. Fallback to Google Trends (Most Searched) if queue is empty
      let topics = [
      ...
      try {
        ...
        if (trendsFeed.items && trendsFeed.items.length > 0) {
          ...
          const eduItem = trendsFeed.items.find((item: any) => 
...
IMPORTANT LANGUAGE RULE: If the topic specifically concerns Non-Hindi speaking states (e.g., Tamil Nadu, Kerala, Karnataka, Andhra Pradesh, Telangana, Maharashtra, West Bengal, Odisha, or North East India), you MUST write the ENTIRE ARTICLE in ENGLISH. Otherwise, write it in HINDI (or Hinglish).
...
5. Transparency Disclaimer: You MUST append the following HTML snippet exactly at the very end of every article to protect the website from Google penalties:
...
- 100% Original and Human-Written. Perfect grammar.
- किसी भी जानकारी का अनुमान नहीं लगाना।${internalLinksContext}${linkContext}�ां केवल "<a href='URL'>👉 Click Here</a>" लिखना।
- जहां जानकारी उपलब्ध न हो वहां "Coming Soon" या "Not Specified" लिखना।
- कोई गलत, अनुमानित या अपुष्ट जानकारी नहीं लिखनी।
- यदि विषय से संबंधित कोई विशेष (extra) जानकारी हो, तो उसे भी Table में नई पंक्ति (Row) बनाकर लिखें।
4. मुख्य विवरण (Main Content): विषय की पूरी जानकारी सरल और विस्तृत भाषा में।
