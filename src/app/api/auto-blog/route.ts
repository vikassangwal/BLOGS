...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!request.headers.get('x-force-run') && !searchParams.get('force') && !searchParams.get('secret')) {
      const lastPost = await prisma.blogPost.findFirst({
        where: { autoGenerated: true },
        orderBy: { createdAt: 'desc' }
      });
      if (lastPost) {
        const elapsedMs = Date.now() - new Date(lastPost.createdAt).getTime();
        const elapsedMinutes = elapsedMs / (1000 * 60);

        let requiredMinutes = 15; // default
        const freq = settings?.frequency || '15m';
        if (freq === '15m') requiredMinutes = 15;
        else if (freq === '30m') requiredMinutes = 30;
        else if (freq === '1h') requiredMinutes = 60;
        else if (freq === '2h') requiredMinutes = 120;
        else if (freq === '4h') requiredMinutes = 240;
        else if (freq === '6h') requiredMinutes = 360;
        else if (freq === '12h') requiredMinutes = 720;
        else if (freq === '24h' || freq === 'daily') requiredMinutes = 1440;
        ...
        if (elapsedMinutes < requiredMinutes) {
          ...
          console.log(`[Auto-Blog] Skipped. Only ${Math.round(elapsedMinutes)} minutes elapsed since last post. Required interval: ${requiredMinutes} minutes. Next run in ${waitTime} minutes.`);
          return NextResponse.json({ 
            success: true, 
            message: `Skipped. Last post was created ${Math.round(elapsedMinutes)} minutes ago. Required interval: ${requiredMinutes} minutes. (Next execution allowed in ${waitTime} minutes.)` 
          });
        }
      }
    }
