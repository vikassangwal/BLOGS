...
export async function POST(request: NextRequest) {
  try {
    ...
    // Frequency spacing control (only for scheduled/cron runs without secret/force override)
    const stepParam = searchParams.get('step') || '';
    const isMultiStageContinuation = stepParam === 'stage2' || stepParam === 'stage3';

    if (!request.headers.get('x-force-run') && !searchParams.get('force') && !searchParams.get('secret') && !isMultiStageContinuation) {
      const lastPost = await prisma.blogPost.findFirst({
        where: { autoGenerated: true, status: 'Published' },
        orderBy: { publishedAt: 'desc' }
      });
      if (lastPost && lastPost.publishedAt) {
        const elapsedMs = Date.now() - new Date(lastPost.publishedAt).getTime();
        const elapsedMinutes = elapsedMs / (1000 * 60);

        const freq = (settings?.frequency || '30m').toLowerCase();
        let requiredMinutes = 30; // default 30 mins to allow multiple posts daily
        if (freq === '15m') requiredMinutes = 15;
        else if (freq === '30m') requiredMinutes = 30;
        else if (freq === '1h' || freq === 'hourly') requiredMinutes = 60;
        else if (freq === '2h') requiredMinutes = 120;
        else if (freq === '4h') requiredMinutes = 240;
        else if (freq === '6h' || freq === 'every6h') requiredMinutes = 360;
        else if (freq === '12h' || freq === 'every12h') requiredMinutes = 720;
        else if (freq === '24h' || freq === 'daily') {
          const maxPerDay = Math.max(1, settings?.maxPostsPerDay || 5);
          requiredMinutes = Math.max(20, Math.floor(1440 / maxPerDay));
        }
        else if (freq === 'weekly') requiredMinutes = 10080;

        // Check daily maximum quota
        const maxPerDay = settings?.maxPostsPerDay || 10;
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);
        const postsToday = await prisma.blogPost.count({
          where: {
            autoGenerated: true,
        ...
        if (postsToday >= maxPerDay) {
          console.log(`[Auto-Blog] Daily limit reached (${postsToday}/${maxPerDay}). Pausing until tomorrow.`);
          return NextResponse.json({
            success: true,
            message: `Daily quota reached (${postsToday}/${maxPerDay} posts published today). Pipeline will resume tomorrow.`
          });
        }

        if (elapsedMinutes < requiredMinutes) {
          const waitTime = Math.ceil(requiredMinutes - elapsedMinutes);
          console.log(`[Auto-Blog] Spacing active. ${Math.round(elapsedMinutes)} minutes elapsed since last post. Required interval: ${requiredMinutes} minutes. Next post allowed in ${waitTime} minutes.`);
          return NextResponse.json({ 
            success: true, 
            message: `Spacing active. Last post was created ${Math.round(elapsedMinutes)} minutes ago. Required interval: ${requiredMinutes} minutes (${postsToday}/${maxPerDay} published today). Next run in ${waitTime} minutes.` 
          });
        }
      }
    }
    ...
    if (step) {
      try {
        if (step === 'stage1') {
          // -------------------------------------------------------------
          // STEP 1: RESEARCH & DRAFT CREATION
          // -------------------------------------------------------------
          ...
          try {
            ...
            if (links.length > 0) {
              liveNewsContext = "LIVE WEB SEARCH RESULTS (Use these exact URLs for Official Notification / Official Website / Apply portal - do NOT guess URLs):\n" + links.join('\n\n');
            }
          } catch (ddgErr) {
            console.error("DDG live search extraction failed in step 1:", ddgErr);
          }

          const researchPrompt = getResearchPrompt(stepTopic, liveNewsContext, customSourceUrl || '', getCurrentDateStr(), getCurrentYearNum());
          let researchData = "";
          try {
            ...
            researchData = `Topic: ${stepTopic}. Provide a comprehensive overview. ${liveNewsContext}`;
          }

          // Create the draft BlogPost
          const post = await prisma.blogPost.create({
            data: {
              title: stepTopic,
              slug: `temp-research-${Math.random().toString(36).substring(2, 6)}-${Date.now().toString().slice(-4)}`,
              content: researchData,
              excerpt: `Research notes for ${stepTopic}`,
              status: 'Researching',
              autoGenerated: true
            }
          });

          // Mark keyword in database
          let kw = await prisma.autoBlogKeyword.findFirst({
            where: { keyword: stepTopic }
          });
          if (!kw) {
            kw = await prisma.autoBlogKeyword.create({
              data: { keyword: stepTopic, status: 'research_completed', postId: post.id }
            });
          } else {
            await prisma.autoBlogKeyword.update({
              where: { id: kw.id },
              data: { status: 'research_completed', postId: post.id }
            });
          }

          return NextResponse.json({ success: true, postId: post.id, keyword: stepTopic });
        }

        if (step === 'stage2') {
          // -------------------------------------------------------------
          // STEP 2: WRITE 1500-WORD HINDI ARTICLE HTML
          // -------------------------------------------------------------
          const postId = searchParams.get('postId') || '';
          if (!postId) return NextResponse.json({ success: false, error: 'postId parameter is required for step 2' });
          ...
          let articleHtml = "";
          try {
            articleHtml = await generateContentWithFallback(writerConfig, "You are a professional Hindi blogger. Output only HTML.", writePrompt);
          } catch (e: any) {
            if (e.message?.includes('429')) {
              return NextResponse.json({ success: false, error: "API Limit (429): AI की फ्री लिमिट खत्म हो गई है या सर्वर बिज़ी है। कृपया 1 घंटे बाद कोशिश करें।" });
            }
        ...
        if (step === 'stage3') {
          // -------------------------------------------------------------
          // STEP 3: SEO METADATA, IMAGES & PUBLISH
          // -------------------------------------------------------------
          const postId = searchParams.get('postId') || '';
          if (!postId) return NextResponse.json({ success: false, error: 'postId parameter is required for step 3' });
          ...
          const seoPrompt = `Analyze the following article HTML and generate SEO metadata. 
          1. seoTitle: Catchy SEO title.
          2. seoDescription: Meta description.
          3. seoKeywords: Keywords.
          4. slug: Clean URL slug.
          5. qualifications: Array of qualifications.
          6. jobStates: Array of states.
          7. applyStartDate: YYYY-MM-DD or null.
          8. applyLastDate: YYYY-MM-DD or null.
          9. vacancyCount: Integer or null.
          10. officialApplyUrl: String or null.
          ...
          let seoData = {
            seoTitle: post.title,
            seoDescription: "Detailed overview of " + post.title,
            seoKeywords: post.title.toLowerCase().split(' ').join(', '),
            slug: post.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-' + Date.now().toString().slice(-4),
          ...
          try {
            ...
            if (parsedSeo.seoTitle) {
              ...
              if (parsedSeo.applyLastDate) {
                ...
                if (!isNaN(lDate.getTime())) seoData.applyLastDate = lDate;
              }
              if (parsedSeo.vacancyCount) seoData.vacancyCount = parseInt(parsedSeo.vacancyCount.toString()) || null;
              if (parsedSeo.officialApplyUrl) seoData.officialApplyUrl = parsedSeo.officialApplyUrl;
            }
          } catch (e) {
            console.error("SEO metadata generation failed, using defaults", e);
          }

          // Link validation & TOC cleaning
          let articleHtml = post.content;
          articleHtml = validateAndFixLinks(articleHtml, post.title);
          articleHtml = cleanTableOfContents(articleHtml, post.title);

          // Image Gen
          const imgPrompt = `Professional educational banner for ${post.title}, high resolution, minimal and clean design`;
          ...
          const updatedPost = await prisma.blogPost.update({
            ...
            data: {
              ...
              gridBox: detectGridBox(post.title, articleHtml),
          ...
          if (kw) {
            await prisma.autoBlogKeyword.update({
              where: { id: kw.id },
              data: { status: 'used', usedAt: new Date(), postId: updatedPost.id }
            });
          }
        ...
        console.error("[Auto-Blog Step Error]", err);
        return NextResponse.json({ success: false, error: err.message || 'Unknown error during step execution' });
      }
    }
...
${links}
    ...
    6. MANDATORY IN-BODY OVERVIEW TABLE (मध्य लेख सारांश सारणी): लेख के बीच में अनिवार्य रूप से <h2>📌 मुख्य भर्ती सारांश सारणी (Recruitment Overview Table)</h2> शीर्षक बनाकर एक सुंदर HTML <table> प्रदान करें जिसमें भर्ती विभाग, पद संख्या, न्यूनतम योग्यता, आयु सीमा, आवेदन तिथि, शुल्क और आधिकारिक लिंक/पीडीएफ की जानकारी हो।
    ...
    12. VIRAL LISTICLE FORMAT: जहाँ भी मुमकिन हो (खासकर Technology और Finance में), जानकारियों को "Top 5", "Top 10", या "Best X" वाले पॉइंट-वाइज़ लिस्ट (Listicle) फॉर्मेट में लिखें। यह रीडर्स को �        --- START MASTER PROMPT ---
        ...
        - NO REPETITION FLUFF: जो जानकारी टेबल में दी गई है, उसे पैराग्राफ में बार-बार दोहराने की ज़रूरत नहीं है। पैराग्राफ में केवल व्यावहारिक नियम, महत्वपूर्ण चेतावनियां, सलाह और प्रक्रिया समझाएं।
        - BANNED WORDS: "आज के इस डिजिटल युग में", "आइए जानते हैं", "निष्कर्ष के तौर पर", "दोस्तों", "रोमांचक". सीधे मुद्दे (Point) पर बात शुरू करें।
        - No Markdown HTML: कंटेंट सीधे पब्लिश करने योग्य HTML फॉर्मेट में होगा (<h2>, <p>, <table>, <ul>, <details>, <summary>)। 
        ...
        - Mobile-First Readability: कोई भी पैराग्राफ 3-4 लाइनों से बड़ा नहीं होना चाहिए। जानकारी को Bullet Points और Tables में व्यवस्थित करें।
        ...
        - Smart Dates: यदि आवेदन या परीक्षा की तिथि घोषित नहीं हुई है, तो केवल "Coming Soon" (जल्द आ रहा है) लिखें या भविष्य के महीनों का अंदाज़ा लगाएं।
        - Highlighting: पूरे लेख में सबसे अहम जानकारी (जैसे: वेतन, अंतिम तिथि, कुल पद) को हमेशा <strong>Bold</strong> करें।
        - Hinglish Keywords (Indian SEO): पैराग्राफ और हेडिंग्स के बीच में नेचुरली 2-3 Hinglish कीवर्ड्स (जैसे: "kaise check kare", "online apply direct link", "notification download") का इस्तेमाल करें।
        
        📝 ब्लॉग का अनिवार्य लेआउट (Blog Structure) - YOU MUST USE THESE EXACT HTML HEADINGS:
        
        <h2>Introduction</h2>
        Write a 150-200 word highly engaging, conversational introduction. Mention the exact advertisement number if available (e.g. Advt No. 01/2026).
        
        <h2>एक नज़र में (Key Highlights)</h2>
        <div class="bg-blue-50 border-l-4 border-blue-500 p-4 my-4 rounded-r">
          <ul> (Write 4 crisp bullet points summarizing: Department, Total Posts, Qualification, Pay Level). </ul>
        </div>
        
        <h2 id="quick-info">Quick Overview (संक्षिप्त विवरण)</h2>
        Create ONE HTML Table combining department name, exact advertisement number, post name, total posts, age cutoff date, and job location.
        
        <h2 id="dates">Important Dates (महत्वपूर्ण तिथियां)</h2>
        Create an HTML Table for dates (Notification release, Online Apply Start, Last Date, Fee Deadline, Correction Window if applicable, Exam Date).
        
        <h2 id="fee">Application Fee (आवेदन शुल्क)</h2>
        Create an HTML Table for category-wise fees (UR/General, OBC/EWS, SC/ST/PwD/Women). Note other-state candidate fee policy.
        
        <h2 id="breakdown">पदों का कंपनीवार / श्रेणीवार विवरण (Vacancy Breakdown Matrix)</h2>
        Create a detailed Column-based HTML Table breaking down posts by Company/Zone/Discipline (e.g. Electrical, Mechanical, Civil, Accounts, Clerical) and categories.
        
        <h2 id="eligibility">शैक्षणिक योग्यता एवं आयु सीमा (Eligibility & Qualifications)</h2>
        Create a clear HTML Table / Bullet List specifying exact qualifications (Degree/12th/B.E./B.Tech) and note explicitly if 3-year diploma is NOT eligible or if specific degree is mandatory. List recognized computer certificates (RSCIT, O-Level, CCC, COPA) if applicable.
        
        <h2 id="salary">वेतनमान एवं प्रोबेशन अवधि (Salary & Probation Period)</h2>
        Create an HTML Table comparing: 1. Pay Level, 2. दो वर्ष के प्रोबेशन में निश्चित मासिक वेतन (Fixed Stipend), 3. प्रोबेशन के बाद न्यूनतम बेसिक पे (Regular Basic Pay).
        
        <h2 id="selection">चयन प्रक्रिया एवं परीक्षा योजना (Selection Scheme & Weightage)</h2>
        Create an HTML Table detailing the exam pattern: Part-A (Subject Weightage %), Part-B (General Aptitude/GK/Reasoning %), Screening Pre marks, and Phase-II Typing test marks (Speed & Efficiency) with minimum qualifying passing marks.
        
        <h2 id="upload-specs">दस्तावेज अपलोड नियम एवं साइज (Document Upload Specifications)</h2>
        Create an HTML Table with specifications: Passport Photo (px & KB + live photo requirement), Signature (px & KB, black ink, no capitals), Thumb Impression (px & KB), Handwritten Declaration (px & KB).
        
        <h2 id="warnings">उम्मीदवारों के लिए जरूरी दिशा-निर्देश (Important Advisories)</h2>
        Provide 4-5 bullet points covering critical guidelines: Company/Zone preference locking, single application rule, preview before final submit, and exam center options.
        
        <h2 id="apply">How to Apply (ऑनलाइन आवेदन कैसे करें)</h2>
        Provide 5 simple, actionable step-by-step points explaining registration, form filling, uploading, and fee payment.
        
        <h2 id="links">Important Links (महत्वपूर्ण लिंक्स)</h2>
        Create an HTML Table with links: Apply Online Link, Download Notification PDF, Official Portal. (Use <a href="..." target="_blank" rel="nofollow" class="font-bold text-blue-600 underline">👉 Click Here</a> for active links).
        
        <h2 id="faq">FAQ (अक्सर पूछे जाने वाले प्रश्न)</h2>
        Provide 2-3 detailed FAQs in <details><summary><strong>[Question]</strong></summary><p>[Detailed Answer]</p></details>.
        ...
        ADD THIS COMMENT HOOK:
        <p class="font-bold text-blue-600 mt-2">💬 <strong>आपकी राय:</strong> इस भर्ती से जुड़ा कोई भी सवाल हो तो नीचे कमेंट करके पूछें!</p>
        --- END MASTER PROMPT ---��से काम करता है? (What is it?)</h2>
    ...
    6. IF the topic is about Education, Jobs, Vacancies, Results, or Career, you MUST strictly follow this 🔥 ULTIMATE MASTER PROMPT 4.0 🔥 format EXACTLY AS WRITTEN:
        --- START MASTER PROMPT ---
        तुम एक Top-Tier Education/Career Content Writer, Pro SEO Expert और Google Discover Specialist हो। 
        
        🚫 कड़े नियम (Strictly Enforced BANNED WORDS & ANTI-REPETITION RULES):
        - CRITICAL RULE: YOU MUST NOT STOP GENERATING. YOU MUST FINISH THE ENTIRE ARTICLE UP TO THE CONCLUSION. NEVER OUTPUT AN INCOMPLETE HTML.
        - NO REPETITION OF TABLE DATA IN PARAGRAPHS: जो जानकारी टेबल में दी गई है, उसे पैराग्राफ में बार-बार दोहराने की ज़रूरत नहीं है। पैराग्राफ में केवल व्यावहारिक नियम, महत्वपूर्ण चेतावनियां, सलाह और प्रक्रिया समझाएं।
        - BANNED WORDS: "आज के इस डिजिटल युग में", "आइए जानते हैं", "निष्कर्ष के तौर पर", "दोस्तों", "रोमांचक". सीधे मुद्दे (Point) पर बात शुरू करें।
        - No Markdown HTML: कंटेंट सीधे पब्लिश करने योग्य HTML फॉर्मेट में होगा (<h2>, <p>, <table>, <ul>, <details>, <summary>)। 
        - Link Format: जहाँ भी कोई आधिकारिक बाहरी लिंक देना हो, वहाँ अनिवार्य रूप से <a href="..." target="_blank" rel="nofollow">👉 Click Here</a> लिखें। 
        - Mobile-First Readability: कोई भी पैराग्राफ 3-4 लाइनों से बड़ा नहीं होना चाहिए। जानकारी को Bullet Points और Tables में व्यवस्थित करें।
        - Highlighting: पूरे लेख में सबसे अहम जानकारी (जैसे: वेतन, अंतिम तिथि, कुल पद) को हमेशा <strong>Bold</strong> करें।
        - Hinglish Keywords: पैराग्राफ और हेडिंग्स के बीच में नेचुरली Hinglish कीवर्ड्स (जैसे: "kaise check kare", "online apply direct link", "notification download") का इस्तेमाल करें।
    ...
    const newPost = await prisma.$transaction(async (tx) => {
      ...
      while (!isUnique) {
        const existing = await tx.blogPost.findFirst({
          where: {
            slug: finalSlug,
            NOT: draftPostId ? { id: draftPostId } : undefined
          }
        });
        if (!existing) {
          isUnique = true;
      ...
      if (draftPostId) {
        post = await tx.blogPost.update({
          ...
          data: {
            ...
            excerpt: seoData.seoDescription,
            featuredImage: featuredImage,
            gridBox: detectGridBox(articleTitle, articleHtml),
            status: settings?.autoPublish ? 'Published' : 'Draft',
            publishedAt: settings?.autoPublish ? new Date() : null,
